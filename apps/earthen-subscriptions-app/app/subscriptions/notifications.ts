...
export async function sendNotification(input: {
  ...
  const configured = input.channel === "email"
    ? Boolean(settings?.emailEnabled && process.env.EMAIL_PROVIDER_URL && process.env.EMAIL_PROVIDER_TOKEN)
    : Boolean(settings?.whatsappEnabled && process.env.HERMES_BASE_URL && process.env.HERMES_TOKEN);
